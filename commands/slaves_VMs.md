...
After initializing the cluster on the master node, Kubernetes generates a join command similar to the following:
...
On each worker node, copy and execute the complete command displayed at the end of the `kubeadm init` output on the master node.

### Notes

* The join command is generated by the master node.
* It allows worker nodes to register themselves with the Kubernetes cluster.
* Each worker node must execute this command only once.
* Port **6443** is the Kubernetes API Server port exposed by the control plane.

If the join command is lost, it can be regenerated on the master node using:
